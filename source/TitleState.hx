...
class TitleState extends MusicBeatState
{
	...
	override public function create():Void
	{
		...
		if(ClientPrefs.checkForUpdates && !closedState) {
			...
			var http = new haxe.Http("https://raw.githubusercontent.com/CPPxfnfmodsmaker/FNF-SPLENDER-NIK-ENGINE/main/gitVersion.txt");

			http.onData = function (data:String)
			{
				updateVersion = data.split('\n')[0].trim();
				var curVersion:String = MainMenuState.splenderinkEngineVersion.trim();
...
backgroundTitle.antialiasing = ClientPrefs.globalAntialiasing;
		...
		FlxTween.tween(ngSpr, {y: ngSpr.y + 50}, 0.6, {ease: FlxEase.quadInOut, type: PINGPONG});

		ngSpr.antialiasing = ClientPrefs.globalAntialiasing;

		//FlxTween.tween(credTextShit, {y: credTextShit.x + 100}, 1, {ease: FlxEase.quadInOut, type: PINGPONG});
	...
	override function beatHit()
	{
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				...
				case 2:
					#if PSYCH_WATERMARKS
					createCoolText(['SPLENDAR NIK Engine by'], 15);
					#else
				...
				case 6:
					#if PSYCH_WATERMARKS
					createCoolText(['Could Be associated', 'with'], -40);
					#else
				...
				// credTextShit.text += '\nNight';
				case 16:
					addMoreText('Funkin SN Engine'); // credTextShit.text += '\nFunkin';
